...
def generate_inputs(assembly, name, study, region_file, fasta_file, bw_file, region_len=65536):
    ...
    rnaseq_file = f'data/{assembly}/{name}/{study}/input_features/tmp/{name}_{region_len}_log_24chr_rnaseq.pt'
    #rnaseq_file = f'data/{assembly}/{name}/{study}/input_features/tmp/{name}_{region_len}_log_21chr_rnaseq.pt'
    ...
    torch.save(torch.Tensor(target), rnaseq_file)
    bw.close()
    
def generate_outputs(assembly, name, study, region_file, fasta_file, bw_file, nBins=1024, region_len=65536):
    regions = pd.read_csv(region_file, sep = '\t', names = ['chr', 'start', 'end', 'strand'])
    riboseq_file = f'data/{assembly}/{name}/{study}/output_features/tmp/{name}_{region_len}_{nBins}_log_24chr_riboseq.pt'
    #riboseq_file = f'data/{assembly}/{name}/{study}/output_features/tmp/{name}_{region_len}_{nBins}_log_21chr_riboseq.pt'
...
if __name__ == '__main__':
    ...
    if args.assembly == "hg38":
        region_file = f'data/{args.assembly}/gene_region_24chr.bed'
        fasta_file = f'data/{args.assembly}/hg38.fa'
    elif args.assembly == "mm10":
        region_file = f'data/{args.assembly}/gene_region_21chr.bed'
...
#python generate_features.py --assembly hg38 --celltype K562 --study GSE153597 --region_len 65536 --nBins 1024