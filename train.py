...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #targets = self.targets[idx].clip(0, 32)
        ...
        #has_input_feat = any(tensor > 0 for tensor in input_features)
        #has_output_feat = any(tensor > 0 for tensor in output_features)
        
        #if has_input_feat or has_output_feat:
        return {
...
class DataModule(LightningDataModule):
    def __init__(
        ...
        **kwargs
        ...
        self.input_features = torch.load(input_file)
        self.output_features = torch.load(output_file)
        ...
        self.train_dataset = Dataset(self.regions.iloc[train_idx], self.input_features[train_idx], self.output_features[train_idx], self.fasta_reader, seq_len, target_length)
        self.val_dataset = Dataset(self.regions.iloc[val_idx], self.input_features[val_idx], self.output_features[val_idx], self.fasta_reader, seq_len, target_length)
        self.test_dataset = Dataset(self.regions.iloc[test_idx], self.input_features[test_idx], self.output_features[test_idx], self.fasta_reader, seq_len, target_length)
    ...
    def train_dataloader(self):
        loader = DataLoader(
            ...
            prefetch_factor=1,
            #collate_fn=self.clean_collate_fn,
        )
        return loader
...
class TrainModule(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = get_cosine_schedule_with_warmup(
            optimizer,
            num_warmup_steps= 2000, #self.hparams.warmup_steps, # 2500
...
if __name__ == '__main__':
    ...
    parser.add_argument('--celltype', dest='dataset_celltype', default='K562',
                        help='Sample cell type for prediction, used for output separation')
    
    parser.add_argument('--study', dest='dataset_study', default='GSE153597',
                        help='GEO number')
  ...
  # Dataloader Parameters
    ...
    #args.default_root_dir = f'results/{args.name}_{args.max_steps}'
    #os.makedirs(args.default_root_dir, exist_ok=True)
    seq_len = SEQ_LEN
    target_length = TARGET_LEN
    region_file = f'data/{args.dataset_assembly}/gene_region_24chr.bed'
    fasta_path = f'data/{args.dataset_assembly}/hg38.fa'
    
    input_file = f'data/{args.dataset_assembly}/{args.dataset_celltype}/{args.dataset_study}/input_features/tmp/{args.dataset_celltype}_{seq_len}_log_24chr_rnaseq.pt'
    output_file = f'data/{args.dataset_assembly}/{args.dataset_celltype}/{args.dataset_study}/output_features/tmp/{args.dataset_celltype}_{seq_len}_{target_length}_log_24chr_riboseq.pt' 
    dataset = DataModule(
    ...
    if args.checkpoint:
        ...
        acc = torch.stack(trainer.test(model, dataset.train_dataloader())).mean(0)
        target_table['pearson_corr_coef-train'] = acc.numpy()
        acc = torch.stack(trainer.test(model, dataset.val_dataloader())).mean(0)
        target_table['pearson_corr_coef-val'] = acc.numpy()
        acc = torch.stack(trainer.test(model, dataset.test_dataloader())).mean(0)
        target_table['pearson_corr_coef-test'] = acc.numpy()
        target_table.to_csv(f'{args.run_save_path}/csv/results.csv', sep='\t')
    else:
        # Early_stopping
